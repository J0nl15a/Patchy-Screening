...
if __name__ == '__main__':
    ...
    abundance_cut = float(sys.argv[5])
    ...
    print(f"[INFO] Starting MCMC with an initial solution at: AMP = {soln.x[0]}, SLOPE = {soln.x[1]}")

    log_prob = log_probability((soln.x[0], soln.x[1]), f_obs, f_obs_err, box, isim, iz, lightcone=lightcone, abundance_cut=abundance_cut, 
                                amp_min=amp_min, amp_max=amp_max, amp_step=amp_step, slope_min=slope_min, slope_max=slope_max, slope_step=slope_step)           
    print(f"[INFO] Log-probability at an initial solution: {log_prob}")
    ...
    f_sim_auto, _, f_sim_auto_var, f_sim_auto_std = emulator(x, 'auto', box, isim, iz, load=True, lightcone=lightcone, 
                                                                    abundance_cut=abundance_cut, amp_min=amp_min, amp_max=amp_max, amp_step=amp_step,
                                                                    slope_min=slope_min, slope_max=slope_max, slope_step=slope_step) 
    f_sim_cross, _, f_sim_cross_var, f_sim_cross_std = emulator(x, 'cross', box, isim, iz, load=True, lightcone=lightcone, 
                                                                       abundance_cut=abundance_cut, amp_min=amp_min, amp_max=amp_max, amp_step=amp_step,
                                                                       slope_min=slope_min, slope_max=slope_max, slope_step=slope_step)
