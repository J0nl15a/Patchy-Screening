...
def data_loader(spectra, box, isim, iz, low_halo_threshold=True, negative_power_threshold=False, shot_noise_included=False, lightcone=0, abundance_cut=0.5,
    ...
    amp_min_name = name_float(amp_min)
    slope_min_name = name_float(slope_min)
    ...
    for i, (a, s) in enumerate(x_train):
        amp_name = name_float(a)
        slope_name = name_float(s)
        print(a,s)
        ...
        if i==0:
            ...
            y_train_abundance = np.array([[number]], dtype=float)
    ...
    if negative_power_threshold:
        x_train = np.delete(x_train, negative_power, axis=0)
        y_train_cross = np.delete(y_train_cross, negative_power, axis=0)
        y_train_auto = np.delete(y_train_auto, negative_power, axis=0)
        y_train_abundance = np.delete(y_train_abundance, negative_power, axis=0)
    print(y_train_cross.shape)
        ...
        for i, (im, slope) in enumerate(x_train):
            ...
            with open(f"./data_files/dndz_samples/{box}/{isim}/{iz}/lightcone{lightcone}/dndz_galaxies_sampled_{im_name}_{slope_name}.txt", "r") as f:
                ...
                if number < nhalo_lower_bound:
                    low_nhalos.append(i)
        x_train = np.delete(x_train, low_nhalos, axis=0)
        y_train_cross = np.delete(y_train_cross, low_nhalos, axis=0)
        y_train_auto = np.delete(y_train_auto, low_nhalos, axis=0)
        y_train_abundance = np.delete(y_train_abundance, low_nhalos, axis=0)
    ...
    if x_train.shape[0] != y_train_auto.shape[0]:
        raise ValueError("Auto-spectrum training rows are misaligned")

    if x_train.shape[0] != y_train_cross.shape[0]:
        raise ValueError("Cross-spectrum training rows are misaligned")

    if x_train.shape[0] != y_train_abundance.shape[0]:
        raise ValueError("Abundance training rows are misaligned")
